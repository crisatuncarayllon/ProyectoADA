...
var ANIMATION_SPEED_DEFAULT = 35;
...
function animWaiting()
{
	...
	objectManager.statusReport.setText("Animacion pausada");
...
function animStarted() 
{
	...
	objectManager.statusReport.setText("Corriendo Animacion");
...
function animEnded()
{
	...
	//	stepBackButton.disabled = false;		
	//}
	objectManager.statusReport.setText("Animacion Completada");
...
//------------------------------------------------------------------------------
    ...
    var controlBar = document.getElementById("GeneralAnimationControls");//no borrar
	
	
	
	var newTable = document.createElement("table");//no borrar
	//se puede borrar desde aqui
	...
	var txtNode = document.createTextNode("Velocidad de animacion"); 
	midLevel.appendChild(bottomLevel);
...
//	heightEntry.size = 4;
	...
	//BOTON SWAP DESACCTIVADO
    //swapButton = addControlToAnimationBar("Button", "cambio de posicion");
    //swapButton.onclick = swapControlDiv;	
	
	
	animationManager.addListener("AnimationStarted", this, animStarted);//ANIMACION INICIADA TEXT 
	animationManager.addListener("AnimationEnded", this, this.animEnded);// ANIMACION TERMINADA
	...
	objectManager.width = canvas.width;//NO BORRAR
	objectManager.height = canvas.height;//NO BORRAR
